...
Usage: $this [-b bindir] [-d] [-q] [-n] [tag]
  ...
  -n turns on dry run mode
   [tag] is a tag from
   https://github.com/shogo82148/dependalint/releases
...
EMBEDDED_CHECKSUMS="
0.1.0:dependalint_0.1.0_darwin_amd64.tar.gz:abfedb4b2083a412a87fbc3e6c70956f07b6b33a9e307dfa71593f3f9e7b3129
0.1.0:dependalint_0.1.0_darwin_arm64.tar.gz:a289f0db5fdeb4251de6a91162b09c8693dc0563a3fb4ebdde884f0a32cc3b61
...
resolve_asset_filename() {
  ...
  if [ -z "${ASSET_FILENAME}" ]; then
    ASSET_FILENAME="dependalint_${VERSION}_${OS}_${ARCH}${EXT}"
  fi
}
...
execute() {
  STRIP_COMPONENTS=0
  CHECKSUM_FILENAME="dependalint_${VERSION}_checksums.txt"
  ...
  BINARY_NAME='dependalint'
  if [ -z "${EXT}" ] || [ "${EXT}" = ".exe" ]; then
    BINARY_PATH="${TMPDIR}/${ASSET_FILENAME}"
  else
    BINARY_PATH="${TMPDIR}/dependalint"
  fi
